...
from core.speech_enhancement_core.SEGAN.SEGAN import get_and_save_enh
from core.speech_enhancement_core.specsub.sub_spec import specsub
from ui.main_widget import Ui_main_widget as m_widget
...
class main_widget(QWidget, m_widget):
    def __init__(self, parent=None):
        ...
        if os.path.exists("./core/speech_enhancement_core/SEGAN/model.pkl"):
            self.save_model_file_path_lineEdit.setText("./core/speech_enhancement_core/SEGAN/model.pkl")
            self.main_data.model_path = "./core/speech_enhancement_core/SEGAN/model.pkl"
    ...
    def plt_save(self):
        ...
        if self.main_data.now_model == self.main_data.model["维纳滤波_spec_sub"] or gl.get_value("is_record"):
    ...
    def choose_model_comboBox_event(self):
        ...
        if self.choose_model_comboBox.currentText() == "维纳滤波_spec_sub" or gl.get_value("is_record"):
    ...
    def play_enh_wav_file_event(self):
        ...
        self.t_play_enh_wav_file = Thread(target=self.play_enh_wav_file)
        self.t_play_enh_wav_file.start()
        # self.play(self.main_data.enh_wav_path)

    def play_enh_wav_file(self, filename=None):
        wf = wave.open(self.main_data.enh_wav_path, 'rb')
        p = pyaudio.PyAudio()
        stream = p.open(format=p.get_format_from_width(wf.getsampwidth()),
                        channels=wf.getnchannels(),
                        rate=wf.getframerate(),
